...
export default function BuyerActivityPage() {
  ...
  return (
    <div className="p-6 space-y-6 bg-claude-cream dark:bg-coal-900">
      {/* Header */}
      <div className="border-b border-claude-border dark:border-coal-700 pb-6">
        <div className="flex items-center space-x-4">
          <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl flex items-center justify-center shadow-lg">
            <Users className="h-7 w-7 text-coal-100" />
          </div>
          <div>
            <h1 className="text-3xl font-bold text-claude-ink dark:text-coal-100">Buyer Activity Monitor</h1>
      ...
      <div className="bg-claude-surface dark:bg-coal-850 rounded-lg shadow-sm border border-claude-border dark:border-coal-700">
        <div className="px-6 py-4 border-b border-claude-border dark:border-coal-700">
          <h2 className="text-lg font-semibold text-claude-ink dark:text-coal-100">Buyers</h2>
        ...
        <div className="p-6">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 dark:border-blue-400"></div>
              <span className="ml-3 text-claude-muted dark:text-coal-400">Loading buyers...</span>
            </div>
          ) : filteredBuyers.length === 0 ? (
            <div className="text-center py-12">
              <User className="h-12 w-12 text-claude-subtle dark:text-coal-500 mx-auto mb-4" />
              <p className="text-claude-muted dark:text-coal-400">No buyers found</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredBuyers.map((buyer) => (
                <Link
                  ...
                  className="group p-4 border border-claude-border dark:border-coal-600 bg-claude-surface dark:bg-coal-700 rounded-lg hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-md transition-all duration-200"
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-claude-ink dark:text-coal-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">
                        {buyer.username}
                      </h3>
                      <p className="text-sm text-claude-subtle dark:text-coal-400 truncate">{buyer.email}</p>
                    </div>
                  </div>
                </Link>
              ))}

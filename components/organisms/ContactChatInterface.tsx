...
export const ContactChatInterface: React.FC<ContactChatInterfaceProps> = ({
  ...
  return (
    <div className="flex h-full bg-claude-surface dark:bg-coal-900 rounded-lg shadow-lg overflow-hidden">
      {/* Left Sidebar - Contact List */}
      <div className="w-80 border-r border-claude-border dark:border-coal-700 flex flex-col bg-claude-cream dark:bg-coal-850">
        {/* Search Header */}
        <div className="py-2 px-4 border-b border-claude-border dark:border-coal-700">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold text-claude-ink dark:text-coal-100">
              Contacts <span className="text-sm font-normal text-claude-subtle dark:text-coal-400">({contacts.length})</span>
            </h2>
            {onNewContact && (
          ...
          <div className="relative">
            <Icon name="search" className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-claude-subtle" />
            <Input
              type="text"
              placeholder="Search contacts..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-10 w-full bg-claude-surface dark:bg-coal-700"
            />
          </div>
        </div>
          ...
          {filteredContacts.length === 0 ? (
            ...
            filteredContacts.map((contact) => (
              <div
                ...
                className={`flex items-center p-3 cursor-pointer hover:bg-claude-sand dark:hover:bg-coal-700 transition-colors ${
              ...
              >
                <div className={`flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center ${
                  selectedContact?.id === contact.id
                    ? 'bg-blue-500 text-coal-100'
                    : 'bg-claude-divider dark:bg-coal-600 text-claude-text dark:text-coal-300'
                }`}>
                ...
                <div className="ml-3 flex-1 min-w-0">
                  <p className={`text-sm font-medium truncate ${
                    selectedContact?.id === contact.id
                      ...
                      : 'text-claude-ink dark:text-coal-100'
                  }`}>
                    {getContactDisplayName(contact)}
                  </p>
                  <p className="text-xs text-claude-subtle dark:text-coal-400 truncate">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {selectedContact ? (
          <>
            {/* Chat Header */}
            <div className="flex items-center justify-between px-4 py-1 border-b border-claude-border dark:border-coal-700 bg-claude-surface dark:bg-coal-850">
              <div className="flex items-center">
                <div className="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center text-coal-100 font-medium">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-sm text-claude-subtle dark:text-coal-400">
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 bg-claude-cream dark:bg-coal-900">
              {messagesLoading && communications.length === 0 ? (
                <div className="flex items-center justify-center h-full">
                  <Icon name="loader-2" className="w-8 h-8 animate-spin text-blue-500" />
                </div>
              ) : communications.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-claude-subtle dark:text-coal-400">
            ...
            {/* Message Input */}
            <div className="p-4 border-t border-claude-border dark:border-coal-700 bg-claude-surface dark:bg-coal-850">
                ...
                <div className="flex items-center space-x-2">
                  <div className="flex-1">
                    <input
                      ...
                      className="w-full h-11 px-4 border border-claude-divider dark:border-coal-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 bg-claude-cream dark:bg-coal-700 text-claude-ink dark:text-coal-100 placeholder-gray-400 dark:placeholder-gray-500"
                    />
                  </div>
                  <Button
          ...
          <div className="flex-1 flex items-center justify-center bg-claude-cream dark:bg-coal-900">
            <div className="text-center text-claude-subtle dark:text-coal-400">
      ...
      {showInfoPanel && selectedContact && (
        <div className="w-80 border-l border-claude-border dark:border-coal-700 bg-claude-surface dark:bg-coal-850 overflow-y-auto">
          {/* Contact Header */}
          <div className="p-4 text-center border-b border-claude-border dark:border-coal-700">
            <div className="h-20 w-20 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center mx-auto mb-4 text-coal-100 text-2xl font-bold">
              {getContactInitials(selectedContact)}
            </div>
            <h3 className="text-xl font-semibold text-claude-ink dark:text-coal-100">
              {getContactDisplayName(selectedContact)}
            </h3>
            {selectedContact.job_title && (
              <p className="text-sm text-claude-subtle dark:text-coal-400">{selectedContact.job_title}</p>
            )}
            {selectedContact.company && (
              <p className="text-sm text-claude-subtle dark:text-coal-400">{selectedContact.company}</p>
            )}
          ...
          <div className="p-4 border-b border-claude-border dark:border-coal-700">
            <h4 className="text-sm font-medium text-claude-subtle dark:text-coal-400 mb-3">Quick Actions</h4>
          ...
          {/* Contact Details */}
          <div className="p-4">
            <h4 className="text-sm font-medium text-claude-subtle dark:text-coal-400 mb-3">Contact Information</h4>
            <div className="space-y-3">
              {selectedContact.mobile && (
                <div className="flex items-center text-sm">
                  <Icon name="smartphone" className="w-4 h-4 text-claude-subtle mr-3" />
                  <div>
                    <p className="text-claude-subtle dark:text-coal-400 text-xs">Mobile</p>
                    <p className="text-claude-ink dark:text-coal-100">{selectedContact.mobile}</p>
                  </div>
                </div>
              )}
              {selectedContact.phone && (
                <div className="flex items-center text-sm">
                  <Icon name="phone" className="w-4 h-4 text-claude-subtle mr-3" />
                  <div>
                    <p className="text-claude-subtle dark:text-coal-400 text-xs">Phone</p>
                    <p className="text-claude-ink dark:text-coal-100">{selectedContact.phone}</p>
                  </div>
                </div>
              )}
              {selectedContact.email && (
                <div className="flex items-center text-sm">
                  <Icon name="mail" className="w-4 h-4 text-claude-subtle mr-3" />
                  <div>
                    <p className="text-claude-subtle dark:text-coal-400 text-xs">Email</p>
                    <p className="text-claude-ink dark:text-coal-100">{selectedContact.email}</p>
                  </div>
                </div>
              )}
              {selectedContact.company && (
                <div className="flex items-center text-sm">
                  <Icon name="building-2" className="w-4 h-4 text-claude-subtle mr-3" />
                  <div>
                    <p className="text-claude-subtle dark:text-coal-400 text-xs">Company</p>
                    <p className="text-claude-ink dark:text-coal-100">{selectedContact.company}</p>
                  </div>
                </div>
              )}
              {selectedContact.assigned_to?.username && (
                <div className="flex items-center text-sm">
                  <Icon name="user" className="w-4 h-4 text-claude-subtle mr-3" />
                  <div>
                    <p className="text-claude-subtle dark:text-coal-400 text-xs">Assigned To</p>
                    <p className="text-claude-ink dark:text-coal-100">{selectedContact.assigned_to.username}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {selectedContact.notes && (
            <div className="p-4 border-t border-claude-border dark:border-coal-700">
              <h4 className="text-sm font-medium text-claude-subtle dark:text-coal-400 mb-2">Notes</h4>
              <p className="text-sm text-claude-text dark:text-coal-300 whitespace-pre-wrap">
                {selectedContact.notes}
              </p>
            </div>
          )}
          ...
          {/* Timestamps */}
          <div className="p-4 border-t border-claude-border dark:border-coal-700 text-xs text-claude-subtle dark:text-coal-400">

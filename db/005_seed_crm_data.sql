...
DO $$
BEGIN
    -- Remove duplicate lead_sources (keep lowest ID) - only if table exists
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'lead_statuses') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contact_types') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'deal_stages') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'deal_categories') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_boards') THEN
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'lead_sources') THEN
        CREATE UNIQUE INDEX IF NOT EXISTS ux_lead_sources_name_ci ON lead_sources (LOWER(name));
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'lead_statuses') THEN
        CREATE UNIQUE INDEX IF NOT EXISTS ux_lead_statuses_name_ci ON lead_statuses (LOWER(name));
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contact_types') THEN
        CREATE UNIQUE INDEX IF NOT EXISTS ux_contact_types_name_ci ON contact_types (LOWER(name));
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'deal_stages') THEN
        CREATE UNIQUE INDEX IF NOT EXISTS ux_deal_stages_name_ci ON deal_stages (LOWER(name));
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'deal_categories') THEN
        CREATE UNIQUE INDEX IF NOT EXISTS ux_deal_categories_name_ci ON deal_categories (LOWER(name));
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'kpi_definitions') THEN
        CREATE UNIQUE INDEX IF NOT EXISTS ux_kpi_definitions_name_ci ON kpi_definitions (LOWER(name));
    END IF;
END $$;
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'task_columns') THEN
